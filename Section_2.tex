...
To model the temporal evolution of atmospheric carbon dioxide ($CO_2$) concentration, a fundamental parameter for structural degradation simulations, an approach based on historical and instrumental records covering the period from 1900 to the present was adopted. The data used to calibrate the model were sourced from renowned international climate research and monitoring associations. For the initial historical period (1900 to 1950), the dataset derives from paleoclimatic records extracted from Antarctic ice cores (Law Dome), managed by the National Centers for Environmental Information of the National Oceanic and Atmospheric Administration (NOAA/NCEI). From 1950 onwards, the model is anchored in continuous direct atmospheric measurements provided by the NOAA Global Monitoring Laboratory (GML) at the Mauna Loa observatory in Hawaii, along with records from the Scripps Institution of Oceanography and the World Data Centre for Greenhouse Gases (WDCGG).

For use in this research, a piecewise analytical regression (incorporating both linear and non-linear behaviors) was created to simulate this $CO_2$ accumulation profile over time, yielding a coefficient of determination ($R^2$) of 0.99. The mathematical framework divides the time series into three continuous regimes of anthropogenic growth: slow linear growth between 1900 and 1950 (rate of 0.30 ppm/year), accelerated linear growth between 1950 and 2000 (rate of 1.16 ppm/year), and quadratic (non-linear) growth post-2000, designed to capture the contemporary intensification of global emissions.

To illustrate the practical application of the described models, parametric simulations were conducted covering the period from 1900 to 2024. The subsequent graphs present the temporal evolution of the carbonation front, highlighting the direct influence of the concrete compressive strength class ($f_{ck}$) and the environmental relative humidity on the degradation rate. For service life assessment purposes, a reference line was set at 30 mm, corresponding to a typical normative concrete cover, indicating the theoretical threshold for reinforcement depassivation
...
Figure \ref{fig:evolution_co2} temporal evolution of carbonation depth for different compressive strength classes ($f_{ck}$), considering a constant relative humidity of 65\% (critical scenario) and a normative concrete cover of 30 mm, simulated for the period from 1900 to 2024.
...
Figure \ref{fig:evolution_h} piecewise regression curve of atmospheric carbon dioxide ($CO_2$) concentration from 1900 to 2024. The graph illustrates the model's transitions between the slow linear historical regime (pre-1950), the accelerated linear regime (1950–2000), and the contemporary quadratic non-linear behavior (post-2000).
