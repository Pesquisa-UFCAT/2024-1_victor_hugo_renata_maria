...
def _process_design_point(
                                x_row: np.ndarray, names_x_variables: list, carb_model: Any,
                                cement_type: int, installation_year: int, exposure_conditions: int,
                                time_step: float, n_latent_samples: int, verbose: bool, i_index: int,
                                years: np.ndarray, calendar_years: np.ndarray, co2_values: np.ndarray, year_query: float
                           ) -> pd.DataFrame:
    """Process a single design point (one row of x): generate its latent samples, run the
    batched carbonation model, fit the GLAM lambdas, and return the resulting DataFrame.

    This is the per-i body of emulator_function_time_durability, factored out as a
    module-level function so it can be sent to worker processes by multiprocessing.Pool.
    """
    ...
    # =========================
    df = pd.DataFrame({
                        names_x_variables[0]: [x_row[0]] * n_latent_samples,
                        names_x_variables[1]: [x_row[1]] * n_latent_samples,
                        names_x_variables[2]: [x_row[2]] * n_latent_samples,
                        'RH_latent': rh_latent,
...
def emulator_function_time_durability(
    ...
    The loop over design points (rows of x) is parallelized across processes with
    multiprocessing.Pool, since each design point is independent of the others.

    :param n_processes: Number of worker processes. If None, uses cpu_count()
    """
    ...
    args_list = [
                    (
                        x[i], names_x_variables, carb_model, cement_type, installation_year, exposure_conditions,
                        time_step, n_latent_samples, verbose, i, years, calendar_years, co2_values, year_query
                    )
                    for i in range(x.shape[0])
                ]
    with Pool(processes=n_processes) as pool:
        dfs = pool.starmap(_process_design_point, args_list)
