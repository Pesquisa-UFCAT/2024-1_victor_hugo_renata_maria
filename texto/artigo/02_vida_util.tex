...
Este trabalho concentra-se na \textbf{fase de iniciação}. O estado limite considerado é, portanto, a \textit{despassivação} da armadura --- o instante em que a frente carbonatada alcança o nível do aço --- e não a ruína estrutural. Trata-se de um estado limite de serviço de durabilidade, para o qual as recomendações do \textcite{fib2006} indicam índices de confiabilidade alvo da ordem de $\beta_{\mathrm{alvo}} \approx 1{,}3$, substancialmente inferiores aos adotados para estados limites últimos, justamente porque sua violação não implica colapso, mas o encerramento do período protegido e o início da janela em que a intervenção de manutenção passa a ser pertinente. Para o limiar nulo, a vida remanescente refere-se ao tempo até a despassivação. Os exemplos também consideram limiares positivos, associados ao prazo até uma intervenção preventiva, conforme a definição da Seção~\ref{subsec:rul_conceito}.
...
\paragraph{Dom\'inio de validade.} O modelo foi calibrado para concretos convencionais brasileiros a partir de ensaios naturais e acelerados, e sua aplicação deve respeitar a faixa de variáveis empregada nessa calibração. O domínio nominal de amostragem adotado neste trabalho é $f_c \in [20;\,50]$~MPa, $UR \in [20;\,80]\%$ e $t \leq 100$~anos, com a concentração de CO$_2$ dada pelas trajetórias da Seção~\ref{subsec:co2_cenarios}. Esse domínio numérico deve ser distinguido do domínio de calibração: além das faixas nominais, as variáveis latentes podem produzir resistências e cobrimentos fora dos intervalos de amostragem das entradas. \falta{confirmar o limite superior de $f_c$ contra a fonte primária: a descrição do modelo em \textcite{possan2010} menciona a faixa de 20 a 40~MPa, enquanto as simulações deste trabalho alcançam 50~MPa --- se a faixa de calibração for mesmo 20--40~MPa, ou se reduz o envelope, ou se declara e justifica explicitamente a extrapolação}
