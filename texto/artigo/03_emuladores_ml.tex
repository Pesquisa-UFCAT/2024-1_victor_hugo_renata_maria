...
Diversos regressores são candidatos naturais a essa tarefa, entre eles as florestas aleatórias (\textit{Random Forests}), a regressão por vetores de suporte (\textit{Support Vector Regression} --- SVR) e as redes neurais artificiais (RNA). A escolha do regressor empregado na camada temporal não é assumida a priori: candidatos são treinados e comparados sobre os próprios parâmetros da GLD, com o desempenho reportado na Seção~\ref{subsec:exemplo2}. \falta{incluir refer\^encias can\^onicas de ML e reconhecimento de padr\~oes, ex.: Hastie et al. (2009); Bishop (2006); Goodfellow et al. (2016), e acrescent\'a-las ao \texttt{references.bib}}
