% =====================================================================
% SECTION 6 - RESULTADOS E DISCUSSAO
% Exemplo 1: benchmark (toy problem) JA EXECUTADO - texto traduzido e
%            condensado a partir da antiga Section_4.
% Exemplo 2: RUL da viga de concreto armado sob carbonatacao - estrutura
%            completa com placeholders em vermelho (a executar).
% Exemplo 3: cenario com manutencao que recupera g - estrutura completa
%            com placeholders em vermelho (a executar).
% =====================================================================
...
\subsubsection{Comportamento temporal dos parâmetros da GLD}
...
\subsubsection{Qualidade da distribuição emulada}
...
% =====================================================================
\subsection{Exemplo 2 --- RUL probabilística de viga de concreto armado sob carbonatação}
\label{subsec:exemplo2}
% =====================================================================

Verificado o arcabouço, aplica-se o emulador ao problema-alvo: a previsão da vida útil remanescente de uma viga de concreto armado sujeita à corrosão induzida por carbonatação, com a função de estado limite da Equação~\eqref{eq:funcao_g} e os modelos físicos das Seções~\ref{sec:carb} e \ref{sec:resist}.
...
\begin{table}[H]
\centering
\caption{Variáveis aleatórias do Exemplo 2. \falta{confirmar distribuições e parâmetros; sugestões baseadas em \citet{jcss2001} e na base de dados da Seção~\ref{subsec:dataset}}}
\label{tab:viga_va}
\begin{tabular}{l c c c c}
\hline
...
\subsubsection{Validação em instantes não vistos}
...
\subsubsection{Evolução da confiabilidade e RUL probabilística}
...
% =====================================================================
\subsection{Exemplo 3 --- Cenário com intervenção de manutenção}
...
\subsubsection{Cenários analisados}

Consideram-se: (i) o cenário de referência sem manutenção (Exemplo~2); (ii) cenários com intervenção única em $t_{\mathrm{man}} \in \{\falta{ex.: 30, 40, 50}\}$ anos, com eficácia de reparo $\rho = \falta{ex.: 0{,}95}$ e coeficiente de carbonatação do material de reparo $k_{\mathrm{rep}} = \falta{valor/hipótese}$; e (iii) o cenário com intervenção no instante normativo $t_{\mathrm{man}}^{*}$ da Equação~\eqref{eq:t_man_otimo}, determinado diretamente da curva $\beta(t)$ emulada para $\beta_{\mathrm{alvo}} = 1{,}5$ \citep{fib2006}. Destaca-se que a avaliação de cada cenário adicional exigiu apenas \falta{valor} segundos de pós-processamento do emulador, sem qualquer nova execução do simulador original --- precisamente o tipo de varredura de cenários que é intratável via Monte Carlo direto.

\subsubsection{Trajetórias e ganho de vida útil}
