...
for i, row in df.iterrows():
    ...
    for j in range(n_simul):
        ...
        m_gk_sample = 6.3235     # np.random.normal(1.06 * m_gk, 0.12 * 1.06 * m_gk)
        ...
        for k in times:
            ...
            m_qk_sample = 0.173   # np.random.gumbel(0.21 * m_qk, 0.21 * 0.76 * m_qk)
            ...
            print(f"[Simulação {j} | Tempo {k} anos]")
            print(f"m_R = {m_rd:.3f}")
            print(f"m_S = {(m_gk_sample + m_qk_sample):.3f}")
            print(f"g = {m_rd - (m_gk_sample + m_qk_sample):.3f}")
            print("-" * 50)
